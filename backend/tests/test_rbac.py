...
async def test_org_create_requires_admin(test_client, db_session):
    ...
    r = await test_client.post("/api/organizations", json=body, headers=await auth_headers(readonly))
    assert r.status_code == 403
    r = await test_client.post("/api/organizations", json=body, headers=await auth_headers(operator))
    assert r.status_code == 403
    r = await test_client.post("/api/organizations", json=body, headers=await auth_headers(admin))
...
async def test_org_update_requires_admin(test_client, db_session):
    ...
    r = await test_client.patch(f"/api/organizations/{org.id}", json=body, headers=await auth_headers(readonly))
    assert r.status_code == 403
    r = await test_client.patch(f"/api/organizations/{org.id}", json=body, headers=await auth_headers(operator))
    assert r.status_code == 403
    r = await test_client.patch(f"/api/organizations/{org.id}", json=body, headers=await auth_headers(admin))
...
async def test_org_list_and_get_allow_readonly(test_client, db_session):
    ...
    r = await test_client.get("/api/organizations", headers=await auth_headers(readonly))
    assert r.status_code == 200
    r = await test_client.get(f"/api/organizations/{org.id}", headers=await auth_headers(readonly))
    assert r.status_code == 200
...
async def test_user_create_requires_global_admin(test_client, db_session):
    ...
    body = {"username": "newuser", "password": "Passw0rd!", "display_name": "New User"}

    # an admin scoped to a single org is not a global admin: user management is global-only
    r = await test_client.post("/api/users", json=body, headers=await auth_headers(org_admin))
    assert r.status_code == 403
    r = await test_client.post("/api/users", json=body, headers=await auth_headers(global_admin))
    assert r.status_code == 201
...
async def test_assign_org_role_requires_global_admin(test_client, db_session):
    ...
    r = await test_client.post(f"/api/users/{target.id}/org-roles", json=body, headers=await auth_headers(operator))
    assert r.status_code == 403
    r = await test_client.post(f"/api/users/{target.id}/org-roles", json=body, headers=await auth_headers(global_admin))
...
async def test_vm_lifecycle_actions_require_operator_or_admin(test_client, db_session):
    ...
    r = await test_client.post(
        f"/api/organizations/{org.id}/deployments/{deployment.id}/power/on", headers=await auth_headers(readonly)
    )
    assert r.status_code == 403
    r = await test_client.delete(
        f"/api/organizations/{org.id}/deployments/{deployment.id}/vm", headers=await auth_headers(readonly)
    )
    assert r.status_code == 403

    # deleting the VM is admin-only; an operator is rejected the same way, before any hypervisor call
    r = await test_client.delete(
        f"/api/organizations/{org.id}/deployments/{deployment.id}/vm", headers=await auth_headers(operator)
    )
    assert r.status_code == 403
...
async def test_dashboard_overview_requires_global_admin(test_client, db_session):
    ...
    r = await test_client.get("/api/dashboard/overview", headers=await auth_headers(org_admin))
    assert r.status_code == 403
    r = await test_client.get("/api/dashboard/overview", headers=await auth_headers(global_admin))
    assert r.status_code == 200
