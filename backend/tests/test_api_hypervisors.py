...
async def test_create_hypervisor_requires_admin(test_client, db_session):
    ...
    r = await test_client.post(
        f"/api/organizations/{org.id}/hypervisors", json=body, headers=auth_headers(readonly)
    )
    assert r.status_code == 403
    r = await test_client.post(
        f"/api/organizations/{org.id}/hypervisors", json=body, headers=auth_headers(operator)
    )
    assert r.status_code == 403

    r = await test_client.post(
        f"/api/organizations/{org.id}/hypervisors", json=body, headers=auth_headers(admin)
    )
...
async def test_list_hypervisors_allows_readonly_and_scopes_by_org(test_client, db_session):
    ...
    await test_client.post(f"/api/organizations/{org_a.id}/hypervisors", json=body, headers=auth_headers(admin_a))

    r = await test_client.get(f"/api/organizations/{org_a.id}/hypervisors", headers=auth_headers(readonly_a))
    assert r.status_code == 200
    assert len(r.json()) == 1

    # readonly_b has no role in org_a, so org-scoped RBAC rejects the request outright
    r = await test_client.get(f"/api/organizations/{org_a.id}/hypervisors", headers=auth_headers(readonly_b))
    assert r.status_code == 403
